...
public class TopicProducer {
    public static void main (String [] args) {
        try {
            ...
            while (true) {
                TextMessage message = session.createTextMessage ("Hello number " + lastMessageCount + " from Java Topic Producer !");
                producer.send (message);
                lastMessageCount ++;
                long thisMessageTime = System.nanoTime ();
                if (thisMessageTime - lastReportTime > 1_000_000_000) {
                    System.out.println ("Sending " + lastMessageCount + " messages per second");
